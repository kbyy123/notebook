...
在RISC-V中有32个寄存器，其RV32变体中，每个寄存器是32位宽；这些32位、4字节的组被称为**字**．若无特殊说明，本课程的RISC-V指的均为其RV32变体．

这32个寄存器编号为**x0~x31**，其中x0的值永远为0．寄存器没有数据类型，操作寄存器的指令决定了我们如何处理寄存器里的数据．
...
**算术运算**：`add x1, x2, x3` 与 `sub x4, x5, x6` 等价于 `x1 = x2 + x3` 与 `x4 = x5 - x6`．由于x0恒为0，因此可以用 `add x3, x4, x0` 达到 `x3 = x4` 的效果（常用于复制数据）． 

汇编语言中的常数被称为**立即数**（immediate），immediate加法有特定的指令：`addi x3, x4, 10`．但是不需要 `subi`，因为 `addi` 操作的立即数可以是负数．
...
用于掩码：与 0x000000FF `andi` 可以提取最低有效字节；如果想提取最高有效字节，通常需要先将数据右移到低位，再配合 `andi`，或者先将掩码加载到寄存器后使用 `and`．

注意到没有 `not`，因为可以与全1做异或操作来翻转每一位的值；在RV32下也可以理解为与32位的全1做异或，因此 `not` 是不必要的．

注意，算术右移在补码意义下可以近似理解为除以2并向下取整；逻辑右移则是单纯在高位补0，不宜直接将其理解为除法．
...
+ **小端序**（Little-Endian）：最低有效字节放在最低地址
+ **大端序**（Big-Endian）：最高有效字节放在最低地址
...
RISC-V的**访存指令**语法：对于load指令是 `op rd, offset(rs1)`，对于store指令是 `op rs2, offset(rs1)`． 
...
+ 参数与返回值寄存器（`a0-a7`，即 `x10-x17`）：向函数传递参数．其中 `a0` 和 `a1` 还可以用于存放函数的返回值．
+ 返回地址寄存器（`ra`，即 `x1`）：专门用于保存函数调用结束后的返回地址，以便程序能回到调用前的位置．
+ 保存寄存器（`s0-s11`，即 `x8-x9, x18-x27`）：通常用于在函数调用期间保存需要保留的数据．
+ 栈指针（sp，即 `x2`）：用于指向栈顶．

### 指令支持

+ 调用函数：使用 `jal`（jump and link），调用函数时通常使用 `jal FunctionLabel` 指令，跳转到目标函数地址，同时将当前指令的下一条指令地址存入 `ra` 中，以便后续返回．
+ 从函数返回：通常写作 `jr`（jump register）这一伪指令，函数执行完毕后使用 `jr ra` 返回．函数返回不能单纯使用 `j` 的固定地址跳转，因为一个函数可能会被多处调用．伪指令 `ret` 等价于 `jr ra`．

实际上，处理这类跳转的真实指令只有：

+ `jal rd, label`（rd表示destination register，用于保存跳转时的返回地址，即跳转前的下一条指令 PC+4）
+ `jalr rd, rs1, imm`（rd保存 PC+4，跳转到 `rs1 + imm`）．
 
`jal FunctionLabel` 相当于 `jal ra, FunctionLabel`；`j Label` 相当于 `jal x0, Label`（相当于直接把返回地址丢弃）；`jr ra` 相当于 `jalr x0, ra, 0`．
...
申请的这块用于存储数据的空间被称为**栈帧**．栈帧存放内容包括参数（超过a0-a7的范围）、局部变量（同参数）、返回地址（如果调用了其他函数）、保存的寄存器（s0-s11，按照约定，如果使用了这些，要在调用前存他们）．
...
本节接触到的RISC-V基础指令都是32位，即每一个字都是一条指令．

<div style="text-align: center; margin-top: 15px;"> 
<img src="assembly-language.assets/image-20260330102445217.png" alt="image-20260330102445217" style="zoom:50%;" />
</div>

程序计数器（program counter）位于处理器内部，其保存着下一条指令的字节地址．通过数据通路（datapath）和内存系统执行完指令后，PC需要指向下一条指令；对于本节接触到的32位定长基础指令，只需 `PC = PC + 4`（字节）即可（或者在分支情况下加载为新地址）．
