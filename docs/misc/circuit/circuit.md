...
**功率**：$p=u\cdot i$​，直接把数据带入（正负号也带入），若为非关联参考方向再加个符号 ． 最后计算结果为正代表吸收功率，为负代表供出功率 ．
...
**基尔霍夫电流定律（KCL）**：对象为结点，流入结点的电流减去流出的电流等于 0． 直接按照这么写，方便后续节点电压法的列式 ．
...
**无源单口网络**等效：若无受控源，则同电阻等效；有受控源时，给单口网络外施电压 $u$，设法求出端口电流 $i$，可等效为 $R=\dfrac{u}{i}$．注意，含有受控源的网络求出的等效电阻可能是负数．
...
<img src="circuit-model.assets/image-20260325165100516.png" alt="image-20260325165100516" style="zoom: 50%;" /> <img src="circuit-model.assets/image-20260325165110873.png" alt="image-20260325165110873" style="zoom: 50%;" />
...
1. 法一：除去独立电源，使用无源单口网络等效方法计算．
2. 法二：将端口用导线连接，计算出导线上的一个短路电流 $i_{sc}$（与计算出的开路电压同向），计算 $R_{0}=\dfrac{u_{oc}}{i_{sc}}$．
